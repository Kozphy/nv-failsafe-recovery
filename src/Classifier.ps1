...
function Get-NvFailsafeClassification {
    ...
    if ($resolution -and $resolution.is640x480) {
        if ($nvidiaPresent) {
            ...
            $classification = 'LOW_RESOLUTION_ONLY'
            $confidence = 0.55
    ...
    elseif ($resolution -and $resolution.isSuspiciouslyLow) {
        ...
        $classification = 'NORMAL_DISPLAY_STATE'
        $confidence = 0.72
        $evidenceItems.Add('Active resolution does not indicate NV-Failsafe fallback.')
        $recommendedNextStep = 'No NV-Failsafe indicators detected; monitor for recurrence after sleep/wake.'
        $tags.Add('NORMAL_DISPLAY_STATE')
    }
    ...
    if ($monitorHandshakeSuspected) {
        ...
        if ($classification -eq 'NORMAL_DISPLAY_STATE') {
    ...
    if ($lastBoot) {
        ...
        if ($bootAge.TotalMinutes -lt 15 -and $classification -ne 'NORMAL_DISPLAY_STATE') {
            $tags.Add('SLEEP_WAKE_DISPLAY_INIT_SUSPECTED')
    ...
    if (($pnpDisabled -gt 0 -or $pnpUnknown -gt 0) -and $classification -ne 'NORMAL_DISPLAY_STATE') {
        $tags.Add('MONITOR_PNP_DRIFT_SUSPECTED')
    ...
    if (-not $isAdmin -and $classification -ne 'NORMAL_DISPLAY_STATE') {
        $tags.Add('ADMIN_REQUIRED')
